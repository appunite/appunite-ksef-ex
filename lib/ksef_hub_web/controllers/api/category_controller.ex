defmodule KsefHubWeb.Api.CategoryController do
  ...
  operation(:index,
    ...
    responses: %{
      200 => {"Category list ordered by sort_order then name", "application/json", Schemas.CategoryListResponse},
  ...
  operation(:create,
    ...
    responses: %{
      ...
      401 => {"Unauthorized — missing or invalid API token", "application/json", Schemas.ErrorResponse},
      422 => {"Validation error — name is required and must be unique within the company", "application/json", Schemas.ErrorResponse}
    }
  )
  ...
  operation(:update,
    ...
    responses: %{
      ...
      401 => {"Unauthorized — missing or invalid API token", "application/json", Schemas.ErrorResponse},
      404 => {"Category not found", "application/json", Schemas.ErrorResponse},
      422 => {"Validation error — name must be unique within the company", "application/json", Schemas.ErrorResponse}
    }
  )
  ...
  operation(:delete,
    ...
    responses: %{
      200 => {"Category deleted — invoices with this category will have it cleared", "application/json", Schemas.MessageResponse},
      401 => {"Unauthorized — missing or invalid API token", "application/json", Schemas.ErrorResponse},
      404 => {"Category not found", "application/json", Schemas.ErrorResponse}
    }
  )
