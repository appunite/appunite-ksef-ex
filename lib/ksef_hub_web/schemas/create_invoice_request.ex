  ...
  OpenApiSpex.schema(%{
    ...
    properties: %{
      ...
      billing_date_from: %Schema{
        ...
        pattern: "^\\d{4}-\\d{2}-01$",
        description:
          "Start of billing period (first day of month, YYYY-MM-01). Auto-computed from sales_date/issue_date if not provided."
      },
      billing_date_to: %Schema{
        ...
        pattern: "^\\d{4}-\\d{2}-01$",
        description:
          "End of billing period (first day of month, YYYY-MM-01). Defaults to billing_date_from for single-month invoices. Must be >= billing_date_from."
      },
      iban: %Schema{
        type: :string,
        nullable: true,
