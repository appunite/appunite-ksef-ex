...
- **No enforced consistency**: Typos or case variations (e.g., "Rent" vs "rent") can create distinct tags. Acceptable because the ML model is the primary source of tag values, not humans. If this becomes a problem, normalize to lowercase on write.
- **No tag descriptions**: The old `Tag.description` field is gone. It was rarely used and not surfaced in the ML pipeline.
- **Performance**: `unnest` + `DISTINCT` for `list_distinct_tags` scans invoices in the company. Adequate for current scale; can add a materialized view if needed later.
